...
import com.yuto.Scientificmagicmod.SMConfig;
import com.yuto.Scientificmagicmod.Api.Event.PlayerMagicPowerEvent;
...
public class MagicPowerStats {
    ...
    public void addStats(EntityPlayer entityPlayer, int par1, float par2) {
        ...
        if (!SMConfig.statusMagicPower) return;
    ...
    public void onUpdate(EntityPlayer par1EntityPlayer) {
        ...
        if (this.MagicPowerExhaustionLevel > 4.0F) {
            this.MagicPowerExhaustionLevel -= 4.0F;

            if (this.MagicPowerSaturationLevel > 0.0F) {
                this.MagicPowerSaturationLevel = Math.max(this.MagicPowerSaturationLevel - 1.0F, 0.0F);
            } else if (i.getDifficultyId() > 0 || SMConfig.peacefulMagicPower) {
                this.MagicPowerLevel = Math.max(this.MagicPowerLevel - 1, 0);
            }
        }
    ...
    public boolean isPacket() {
        ...
        if (!SMConfig.statusMagicPower) return false;
    ...
    public int getMagicPowerLevel() {
        ...
        if (!SMConfig.statusMagicPower) return MAX_MagicPower_LEVEL;
