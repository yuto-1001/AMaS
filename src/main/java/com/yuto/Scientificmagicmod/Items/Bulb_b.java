...
public class Bulb_b extends ItemSeedFood {

	public Bulb_b() {
		super(30,30,ScientificmagicBlock.CA_b, Blocks.grass);
		String name = "Bulb_b";
        this.setUnlocalizedName( name );
        maxStackSize = 64;
        this.setCreativeTab( ScientificmagicMod.Scientificmagicmod );
        this.setTextureName( "scientificmagicmod:Bulb_bl" );
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		if (side != 1) {
			...
			if (world.getBlock(x, y, z).canSustainPlant(world, x, y, z, ForgeDirection.UP, this) && world.isAirBlock(x, y + 1, z)) {
				// 作物を設置する。
				world.setBlock(x, y + 1, z, ScientificmagicBlock.CA_b);
				// スタック数を減らす。
				--itemStack.stackSize;
				return true;
			} else {
